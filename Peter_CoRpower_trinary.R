#' @param noobsatriskmotaucasesALL  Number of subjects in the vaccine group at-risk at tau and with the clinical event (cases) by taumax (regardless of whether the biomarker is measured).
#' @param noobsatriskmotaucasesPhase2 Number of subjects in the vaccine group at-risk at tau and with the clinical event (cases) by taumax and with the biomarker measured (i.e., in Phase 2).
#' @param noobsatriskmotaucontrolsALL Number of subjects in the vaccine group at-risk at tau and without the clinical event (controls) by taumax (regardless of whether the biomarker is measured).
...
#' noobsatriskmotaucasesALL <- 41
#' noobsatriskmotaucontrolsALL <- 7662
#' noobsatriskmotaucasesPhase2 <- 41
#' risk1 <- noobsatriskmotaucasesALL/(noobsatriskmotaucasesALL + noobsatriskmotaucontrolsALL)
...
#' computepower(noobsatriskmotaucasesALL, noobsatriskmotaucasesPhase2, noobsatriskmotaucontrolsALL,
...
#' computepower(noobsatriskmotaucasesALL, noobsatriskmotaucasesPhase2, noobsatriskmotaucontrolsALL, risk0,
...
#' @import osDesign
#' @export
computepower <- function(noobsatriskmotaucasesALL, noobsatriskmotaucasesPhase2, noobsatriskmotaucontrolsALL,
                         risk0, RRoverall,
                         Plat0, Plat2,
                         P0=0.3, P2=0.2,
...
nALL <- noobsatriskmotaucasesALL
nPhase2 <- noobsatriskmotaucasesPhase2
...
nnALL <- nALL + noobsatriskmotaucontrolsALL
...
if (Approach2) {
  ...
  x1 <- c(sigma2obs,rhos[1],Plat0,P0,Plat2,P2,Sens1,Spec1,FP11,FN11,FP21,FN21)
  #cat("sigma2obs,rho,Plat0,P0,Plat2,P2,Sens,Spec,FP1,FN1,FP2,FN2 in order of the vector rhos")
  #cat("\n")
  #cat(paste(round(x1,3)),"\n")
  
  x2 <- c(sigma2obs,rhos[2],Plat0,P0,Plat2,P2,Sens2,Spec2,FP12,FN12,FP22,FN22)
  #cat("\n")
  #cat(paste(round(x2,3)),"\n")
  
  x3 <- c(sigma2obs,rhos[3],Plat0,P0,Plat2,P2,Sens3,Spec3,FP13,FN13,FP23,FN23)
  #cat("\n")
  #cat(paste(round(x3,3)),"\n")
  
  x4 <- c(sigma2obs,rhos[4],Plat0,P0,Plat2,P2,Sens4,Spec4,FP14,FN14,FP24,FN24)
  #cat("\n")
...
if (!Approach2) { #*# use given Sens, Spec, FP1, and FN1 params
  ...
  FN21 <- (P0 - Spec1*Plat0 - FN11*Plat2)/Plat1  #*#P0, Plat0, Plat2 given params
...
# Observed risks P(Y(1)=1|S(1)=lo, med, or hi)  #*# for diff values of rho; using Bayes' rule
probXlo_cond_Shi <- FP11*Plat0/P2
probXmed_cond_Shi <- FP21*Plat1/P2
probXhi_cond_Shi <- Sens1*Plat2/P2
risk1hi1 <- (RRlat0*probXlo_cond_Shi + RRlat1*probXmed_cond_Shi + RRlat2*probXhi_cond_Shi)*risk0
probXlo_cond_Slo <- Spec1*Plat0/P0
probXmed_cond_Slo <- FN21*Plat1/P0
probXhi_cond_Slo <- FN11*Plat2/P0
risk1lo1 <- (RRlat0*probXlo_cond_Slo + RRlat1*probXmed_cond_Slo + RRlat2*probXhi_cond_Slo)*risk0
...
risk1hi3 <- (RRlat0*probXlo_cond_Shi + RRlat1*probXmed_cond_Shi + RRlat2*probXhi_cond_Shi)*risk0
probXlo_cond_Slo <- Spec3*Plat0/P0
probXmed_cond_Slo <- FN23*Plat1/P0
probXhi_cond_Slo <- FN13*Plat2/P0
risk1lo3 <- (RRlat0*probXlo_cond_Slo + RRlat1*probXmed_cond_Slo + RRlat2*probXhi_cond_Slo)*risk0
...
risk1hi4 <- (RRlat0*probXlo_cond_Shi + RRlat1*probXmed_cond_Shi + RRlat2*probXhi_cond_Shi)*risk0
probXlo_cond_Slo <- Spec4*Plat0/P0
probXmed_cond_Slo <- FN24*Plat1/P0
probXhi_cond_Slo <- FN14*Plat2/P0
risk1lo4 <- (RRlat0*probXlo_cond_Slo + RRlat1*probXmed_cond_Slo + RRlat2*probXhi_cond_Slo)*risk0
risk1med4 <- (risk1 - risk1lo4*P0 - risk1hi4*P2)/P1

# Note: For the biomarker special case, the risk1medx are NA

#*# effect size of the biomarker-specific risk ratio, RR_t
#*# length is equal to length(RRlat0)
...
if (o>0){
  nus <- sqrt(rhos*sigma2obs)*qnorm(PlatVElowest) #*# latent quantile value for lowerst VE cohort cutoff 
  truebetas <- rep(NA,o)
  alphalatvect <- rep(NA,o)
  
  for (l in 1:o) {
    
    risk1latnu <- (1-VElowestvect[l])*risk0  #*# constant risk function for x < nu
    
    f <- function(alpha) {
    
      g <- function(x) { #*# Appendix B
        rho <- 1
...
risk1cont <- function(x,alphalat,betalat) { #*# computes inverse-logit of (alphalat + betalat*x)
...
for (i in 1:M) { #*# number of simulations
  ...
  for (j in 1:length(esvect1)) {
    ...
    if (!zerosflag) { #*# if no zeros, use logistic regression ordered score test
...
#*# saving power calculations
#*# power defined as fraction of simulated trials with Wald test 2-sided p-value <= alpha
...
anstrin <- cbind(powerstrinaryrho1,powerstrinaryrho2,powerstrinaryrho3,powerstrinaryrho4) #*# trinary
anscont <- cbind(powerscontrho1,powerscontrho2,powerscontrho3,powerscontrho4) #*# continuous
if (o > 0)
...
computeSensSpecFPFN <- function(sigma2obs,rho,Plat0,Plat2,P0,P2) {
  ...
  #*# When used in the computepower function, P0 and P2 are scalar 
  ...
  if (rho < 1) {  #*# if rho=1, then Sens=1, Spec=1, FP1=0, FP2=0, FN1=0, FN2=0
    # Stochastic integration
    X <- rnorm(20000,0,sqrt(sigma2tr))
    S <- X + rnorm(20000,0,sqrt(sigma2e))
    ...
    for (i in 1:length(tauhi)) {
      Sensvec[i] <- (sum(S>tauhi[i] & X > thetahiVE)/length(S))/Phi
      Specvec[i] <- (sum(S<=taulo[i] & X <= thetaloVE)/length(S))/Plo
      if (Pmed==0) { #*# if binary biomarker, 0's for FP2, FP1, FN2, FN1
        ...
        FN2vec[i] <- 0 }
      if (Pmed > 0) {
        FP2vec[i] <- (sum(S>tauhi[i] & X > thetaloVE & X <= thetahiVE)/length(S))/Pmed
        FP1vec[i] <- (sum(S>tauhi[i] & X <= thetaloVE)/length(S))/Plo
        FN1vec[i] <- (sum(S<=taulo[i] & X > thetahiVE)/length(S))/Phi
        FN2vec[i] <- (sum(S<=taulo[i] & X > thetaloVE & X <= thetahiVE)/length(S))/Pmed
      }
    }
