---
title: 'R Package `CoRpower`: Power and Sample Size Calculations for Correlates of
  Risk'
...
## Calculate $N$, $n_{cases}$, $n_{controls}$, and $n^S_{cases}$ | Without replacement sampling {.smaller}
```{r, include=FALSE}
...
computeN(tau = 3.5,          
         ...
         NvEnrolled = 4100,  # number of participants enrolled and randomized to vaccine arm
         risk0 = 0.034,      # cumulative placebo risk between tau and tau_max
         dropOutRate = 0.1,  # cumulative dropout rate between 0 and tau_max
         fracImmunog = 1.0)  # fraction of cases selected for the immunogenicity subset
```
...
### Plotting of $RR_2^{lat}/RR_0^{lat}$ vs. $RR_t$
```{r, eval=FALSE}
plotRRgradVE(outComputePower = list(pwr),   # output list from 'computePower'
...
pwr <- computePower(nCases = c(25, 32, 35, 40),             
                    nControls = c(3998, 3585, 3988, 3983),  
...
Other components: `betaLat`, `alphaLat`, `PlatVElowest`, `N`, `nCases`, `nCasesPhase2`, `VEoverall`, `alpha`, `rho`, `controlCaseRatio`

## *Scenario 8:* vary \(\small\, \rho \) | Continuous \(\small\, S^{\ast}\), without replacement sampling {.smaller}
### Plotting of VE curves for different relative risks
```{r, eval=FALSE}
plotVElatContinuous(outComputePower = list(pwr),  # output list from 'computePower'
                      risk0 = 0.034,                # cumulative placebo risk between tau and tau_max
                      legendText = paste0("rho = ", c(1, 0.9, 0.7, 0.5)))
```
...
```{r, eval=FALSE}
computePower(..., saveDir = "myDir", saveFile = "myFile.RData")
plotVElatContinuous(outComputePower = "myFile",   # file with 'computePower' output
                      outDir = "~/myDir",           # path to myFile.RData
                      risk0 = 0.034,                # cumulative placebo risk between tau and tau_max
                      legendText = paste0("rho = ", c(1, 0.9, 0.7, 0.5)))
```
...
### Plotting of power curves
```{r, eval=FALSE}
plotPowerContinuous(outComputePower = list(pwr),    # output list from 'computePower'
                    legendText = paste0("rho = ", c(1, 0.9, 0.7, 0.5)))
```
...
```{r, eval=FALSE}
computePower(..., saveDir = "myDir", saveFile = "myFile.RData")
plotPowerContinuous(outComputePower = "myFile",     # file with 'computePower' output
                    outDir = "~/myDir",             # path to myFile.RData
                    legendText = paste0("rho = ", c(1, 0.9, 0.7, 0.5)))
```
