...
### **Trial design**
- $N_{rand} = 4,600$ participants randomized to each of the vaccine and placebo group and followed for $\tau_{\mathrm{max}}=$ 24 months
- Samples for measurement of $S$ at $\tau=$ 3.5 months stored in all vaccine recipients
- Cumulative endpoint rate between $\tau$ and $\tau_{\mathrm{max}}$ in placebo group $=2.9\%$ $(=risk_0)$
...
- $N = N_{rand} \times P(T>\tau, C>\tau \mid Z=1) \approx 4,516$
- $n_{cases} = N \times P(T\leq \tau_{\mathrm{max}}, T\leq C \mid T>\tau, C>\tau, Z=1) \approx 31$
- $n^S_{cases} = n_{cases}$
...
## Stephanie: Would you please insert here code chunks and output/figures/tables for Scenarios 1--6 from the previous slide?

## Illustration: `CoRpower` for binary \(\small\, S\) | Without replacement sampling
Accomplished by selecting $P_0^{lat}$, $P_2^{lat}$, $P_0$, $P_2$ such that
\[
\begin{align}
P_0^{lat} + P_2^{lat} &= 1\\
P_0 + P_2 &= 1
...
## Stephanie: Would you please insert here the code chunk and output/figure for Scenario 7 from the previous slide?

## To do for Michal: Slides on algorithm for a continuous biomarker and without replacement sampling

## Illustration: `CoRpower` for continuous \(\small\, S\) | Without replacement sampling
- *Scenario 8:* vary $\rho$
- *Scenario 9:* vary $P_{lowestVE}^{lat}$

## Stephanie: Would you please insert here code chunks and output/figures/tables for Scenarios 8--9 from the previous slide?

## To do for Michal: Slides on Bernoulli sampling of both a trichotomous and continuous marker

## Illustration: `CoRpower` for trichotomous \(\small\, S\) | Bernoulli sampling
- *Scenario 10:* vary the probability of sampling into the subcohort at baseline

## Stephanie: Would you please insert here the code chunk and output/figures for Scenario 10 from the previous slide?

## Illustration: `CoRpower` for continuous \(\small\, S\) | Bernoulli sampling
- *Scenario 11:* vary the probability of sampling into the subcohort at baseline

## Stephanie: Would you please insert here the code chunk and output/figures for Scenario 11 from the previous slide?
...
RRlat2[1:5]
```
<li>`risk1_2`, `risk1_0`, `RRlat0`, `Plat2`, `Plat0`, `P2`, `P0`, `trinaryalpha`, `trinarybeta`, `Sens`, `Spec`, `FP0`, `FN2`, `N`, `nCases`, `nPhase2`, `VEoverall`, `alpha`, `rhos`, `controlCaseRatio`
</ul>
...
```{r, eval=FALSE}
ans <- computepower(numAtRiskTauCases=c(25, 31, 35, 40), 
