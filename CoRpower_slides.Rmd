...
## *Scenario 1:* vary control:case ratio (Approach 1) {.smaller}
Can store `computePower` output in an .RData file by specifying:
<br>
<br>
<ul>
  <li> `saveDir`: file directory
  <li> `saveFile`: file name
<br>
Then, use saved output for plotting:

```{r, eval=FALSE}
plotPowerTrinary(dataDir = c("~/Scen1/1", "~/Scen1/K2", "~/Scen1/K3"), # input directories 
                 dataFile = c("pwr1","pwr2","pwr3"),                   # input files (.RData files)
                 variableName = "controlCaseRatio",                    # name of varying parameter
                 variableValues = c(5, 3, 1),                          # values of varying parameter
