...
```{r, eval=FALSE}
library(CoRpower)
pwr <- computepower(nAtRiskTauCases = 31,             # n_cases
                    nAtRiskTauControls = 4485,        # n_controls
                    nAtRiskTauCasesPhase2 = 31,       # num^S_cases
...
## *Scenario 2:* Trichotomous, approach 1 | Vary \(\small\, Sens \) and \(\small\, Spec\) {.smaller}
```{r, eval=FALSE}
pwr <- computepower(nAtRiskTauCases = 31,             # n_cases
                    nAtRiskTauControls = 4485,        # n_controls
                    nAtRiskTauCasesPhase2 = 31,       # num^S_cases
                    controlCaseRatio = 5,             # ratio of controls to cases
                    RRoverall = 0.25,                 # 1 - overall VE
                    risk0 = 0.029,                    # cumulative placebo risk between tau and tau_max 
                    RRlat0 = seq(1, 0.25, len = 100), # grid of relative risk (V/P) among lower protected
                    RRlat1 = rep(0.25, 100),          # grid of relative risk (V/P) among medium protected
...
## *Scenario 3:* Trichotomous, approach 1 | Vary \(\small\, P^{lat}_0, P_0, P^{lat}_2, P_2\) {.smaller}
```{r, eval=FALSE}
pwr <- computepower(nAtRiskTauCases = 31,             # n_cases
                    nAtRiskTauControls = 4485,        # n_controls
                    nAtRiskTauCasesPhase2 = 31,       # num^S_cases
                    ...
                    RRlat0 = seq(1, 0.25, len = 100), # grid of relative risk (V/P) among lower protected
                    RRlat1 = rep(0.25, 100),          # grid of relative risk (V/P) among medium protected
                    Plat0 = 0.05,                     # scalar, ie separate 'computepower' calls for 0.05, 0.1, 0.15, 0.2
                    Plat2 = 0.15,                     # scalar, ie separate 'computepower' calls for 0.15, 0.3, 0.45, 0.6
                    P0 = 0.05,                        # scalar, ie separate 'computepower' calls for 0.05, 0.1, 0.15, 0.2
                    P2 = 0.15,                        # scalar, ie separate 'computepower' calls for 0.15, 0.3, 0.45, 0.6
                    Sens = 0.8, Spec = 0.8, FP0 = 0, FN2 = 0,
                    M = 1000,                         # number of simulated clinical trials
                    alpha = 0.05,                     # two-sided Wald test Type 1 error rate
                    biomType = "trichotomous")        # "continuous" by default
