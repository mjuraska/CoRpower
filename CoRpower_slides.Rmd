...
```
Other components: `risk1_2`, `risk1_0`, `VElat0`, `Plat2`, `Plat0`, `P2`, `P0`, `alphaLat`, `betaLat`, `sens`, `spec`, `FP0`, `FN2`, `N`, `nCases`, `nCasesPhase2`, `VEoverall`, `alpha`, `rho`, `controlCaseRatio`
...
```{r, eval=FALSE}
ans <- computePower(nAtRiskTauCases = 32,                # n_cases
                    nAtRiskTauControls = 3991,           # n_controls
                    nAtRiskTauCasesPhase2 = 32,          # n^S_cases
                    controlCaseRatio = 5,                # n^S_controls : n^S_cases ratio
                    VEoverall = 0.75,                    # overall VE
                    ...
                    rho = c(1, 0.9, 0.7, 0.5),           # fraction of relevant variance in the observed biomarker
                    biomType = "trichotomous")           # "continuous" by default
```
...
ans <- computePower(nAtRiskTauCases = 32,                # n_cases
                    ...
                    alpha = 0.05,                        # two-sided Wald test Type 1 error rate
                    sigma2obs = 1,                       # variance of observed biomarker
                    rho = 0.9,                           # fraction of relevant variance in the observed biomarker
                    biomType = "trichotomous")           # "continuous" by default
```
...
```{r, eval=FALSE}
ans <- computePower(nAtRiskTauCases = c(25, 32, 35, 40),      # n_cases
                    nAtRiskTauControls=c(3998,3991,3988,3983),# n_controls
                    ...
                    alpha = 0.05,                             # two-sided Wald test Type 1 error rate
                    sigma2obs = 1,                            # variance of observed biomarker
                    rho = 0.9,                                # fraction of relevant variance in observed biomarker
                    biomType = "trichotomous")                # "continuous" by default
```
...
```{r setup}
knitr::opts_knit$set(root.dir = "T:/vaccine/StephanieWu/CoR Power Package In Progress/Scenario9")
```
```{r, include=FALSE, cache=TRUE}
load("ansScen9.RData")
```
...
## *Scenario 10:* vary cohort sampling probability \(\small\, p \) (Approach 1) | Trichotomous \(\small\, S \), Bernoulli sampling {.smaller}
```{r, eval=FALSE}
ans <- computePower(nAtRiskTauCases = 32,                # n_cases
                    nAtRiskTauControls = 3991,           # n_controls
                    nAtRiskTauCasesPhase2 = 32,          # n^S_cases
                    ...
                    alpha = 0.05,                        # two-sided Wald test Type 1 error rate
                    biomType = "trichotomous",           # "continuous" by default
                    cohort = TRUE,                       # case-cohort sampling design
                    p = 0.01,                            # scalar, ie separate calls for 0.01, 0.02, 0.03, 0.05
                    pDropout = 0.1)                      # probability of dropout between tau and tau_max       
```

## *Scenario 10:* vary cohort sampling probability \(\small\, p \) (Approach 1)
```{r, echo=FALSE, fig.align='center', out.width='65%'}
knitr::include_graphics(file.path(figDir,"powertrinary_CoR_Scen4.png"))
```


## *Scenario 11:* vary cohort sampling probability \(\small\, p \) | Continuous \(\small\, S^{\ast}\), Bernoulli sampling {.smaller}
```{r, eval=FALSE}
ans <- computePower(nAtRiskTauCases = 32,                 # n_cases
                    nAtRiskTauControls = 3991,            # n_controls
                    nAtRiskTauCasesPhase2 = 32,           # n^S_cases
...
## *Scenario 11:* vary cohort sampling probability \(\small\, p \)
```{r, echo=FALSE, fig.align='center', out.width='65%'}
...
Gilbert, Janes, Huang (2016, *Stat Med*)
</div>