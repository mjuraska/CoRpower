...
<ul>
<li> We have a trichotomous $X$ with a user-specified distribution defined by $P^{lat}_x$, $x=0,1,2$ (via input arguments `Plat0` and `Plat2`), which implies that
$$
\var X = \sum_{k=0}^2 k^2 P^{lat}_k - \left( \sum_{k=0}^2 k P^{lat}_k \right)^2
$$
<li> Next, we have a trichotomous $S(1)$ with a user-specified distribution defined by $P_s$, $s=0,1,2$ (via input arguments `P0` and `P2`), where
$$
P_s  = \sum_{k=0}^2 P\{S(1)=s \mid X=k\} P^{lat}_k, \quad s=0,1,2.
$$
