...
plotVEcurveContinuous <- function(dataDir, dataFile, variableName, variableValues, legendText=NULL, risk0) {
  ...
  # Compute VE(s_1) vs s_1 for each fixed RRs value:
  ...
  # postscript("/home/pgilbert/manuscripts/Li/CoRpowercalculations/Figures/plotVEcurveCoRcontinuous.eps",horizontal=T)
  par(cex.axis=1.4,cex.lab=1.4,cex.main=1.35,mar=c(4,5,4,4),oma=c(3,0,3,0),las=1)
  plot(svect,VEcurverr1,xlim=c(-3,3),ylim=c(-0.05,1),type='n',xlab="True Biomarker X*=x* in Vaccinees",ylab="VE(x)",axes=FALSE)
  axis(1)
  axis(2)
  ...
  legend(x="bottomright",legend=c(as.expression(bquote("RR=1.00, "~VE[lowest]==.(round(VElowest[inds[8]],2)))),
                                  as.expression(bquote("RR="~.(round(FixedCoRRRs[7],2))~", "~VE[lowest]==.(round(VElowest[inds[7]],2)))),
                                  as.expression(bquote("RR="~.(round(FixedCoRRRs[6],2))~", "~VE[lowest]==.(round(VElowest[inds[6]],2)))),
                                  as.expression(bquote("RR="~.(round(FixedCoRRRs[5],2))~", "~VE[lowest]==.(round(VElowest[inds[5]],2)))),
                                  as.expression(bquote("RR="~.(round(FixedCoRRRs[4],2))~", "~VE[lowest]==.(round(VElowest[inds[4]],2)))),
                                  as.expression(bquote("RR="~.(round(FixedCoRRRs[3],2))~", "~VE[lowest]==.(round(VElowest[inds[3]],2)))),
                                  as.expression(bquote("RR="~.(round(FixedCoRRRs[2],2))~", "~VE[lowest]==.(round(VElowest[inds[2]],2)))),
                                  as.expression(bquote("RR="~.(round(FixedCoRRRs[1],2))~", "~VE[lowest]==.(round(VElowest[inds[1]],2))))),
         cex=1.2,lty=c(1,2,3,4,5,6,7,8),col=c("brown","blue","dark grey","orange","red","purple","green","black"),lwd=2)
  title("VE Curves for Different CoR Relative Risks per 1 SD in Vaccinees")
  text(2, 0.46, bquote(atop(VE[overall]==.(round(VEoverall,2))~", "~risk[o]==.(round(risk0,3)),
                            ~P[lowest]^{lat}==.(round(PlatVElowest,2)))),cex=1.3)
